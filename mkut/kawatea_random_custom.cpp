...
const double MAIN_TIME_LIMIT = 10;
...
int tmp_blocked_count[MAX_ATTENDEE][MAX_MUSICIAN];
vector<geo::P> best_placements;

void input() {
    manarimo::load_problem(std::cin, problem);
    cerr << problem.playing_together << endl;
...
set<int> move(vector<geo::P>& placements, int idx, double dx, double dy, set<int>& moved) {
    if (moved.count(idx)) return moved;
    moved.insert(idx);
    if (placements[idx].first + dx < stage_left) return set<int>();
    if (placements[idx].first + dx > stage_right) return set<int>();
    if (placements[idx].second + dy < stage_bottom) return set<int>();
    if (placements[idx].second + dy > stage_top) return set<int>();
    placements[idx].first += dx;
    placements[idx].second += dy;
    for (int i = 0; i < placements.size(); i++) {
        if (i == idx) continue;
        if (dist2(placements[i], placements[idx]) < RADIUS2) {
            moved.insert(i);
            if (move(placements, i, dx, dy, moved).size() == 0) {
                return set<int>();
            }
        }
    }
    ...
    for (int times = 1; times > 0; --times) {
        ...
        while (!sa.end()) {
            ...
            if (random::get(100) < 80) {
                ...
                set<int> moved;
                move(next_placements, _m, dx, dy, moved);
                if (moved.size() == 0) continue;
                for (int m : moved) {
                    next_placements[m].first += dx;
                    next_placements[m].second += dy;
                }
                ...
                if (sa.accept(current_score, next_score)) {
                    current_score = next_score;
                    swap(attendee_angles, tmp_attendee_angles);
                    swap(blocked_attendees, tmp_blocked_attendees);
                    for (int m : moved) {
