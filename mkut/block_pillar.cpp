...
double score_all_exact() {
    ...
    for (int i = 0; i < problem.musicians.size(); i++) {
        ...
        for (int j = 0; j < problem.attendees.size(); j++) {
            if (blocked_count[i][j] == 0) sum += ceil(q * calc_one_score(placements[i], problem.attendees[j].pos, problem.attendees[j].tastes[problem.musicians[i]]));
        }
    }
    return sum;
}
...
void move(double dx, double dy, int idx, set<int>& moved) {
    if (moved.count(idx)) {
        return;
    }
    moved.insert(idx);
    geo::P p = placements[idx];
    geo::P next_p = p;
    double dx2 = clamp(dx, stage_left - p.X, stage_right - p.X);
    double dy2 = clamp(dy, stage_bottom - p.Y, stage_top - p.Y);
    next_p.first += dx2;
    next_p.second += dy2;
    bool ng = false;
    for (int i = 0; i < placements.size(); i++) {
    ...
    if (!ng) {
        placements[idx] = next_p;
        //cerr << "moved (" << idx << "): " << dx2 << "," << dy2 << endl;
    } else {
        //cerr << "not moved (" << idx << ")" << endl;
    }
}
...
int main(int argc, char *argv[]) {
    ...
    double input_score = 0;
    if (argc < 2) {
        sa_no_block();
        placements = best_placements;
    } else {
        int rnd = clock() % 10000;
        for (int i = 0; i < rnd; i++) random::get(1);
        manarimo::solution_t intermediate_solution;
        manarimo::load_solution(string(argv[1]), intermediate_solution);
        double left_limit = 0, right_limit = 0, bottom_limit = 0, top_limit = 0;
        best_placements = intermediate_solution.as_p();
        placements = best_placements;
        input_score = score_all_approximate();
        fprintf(stderr, "input_score : %lf\n", input_score);
        double r = 1;;//random::get_double(0, 1);
    ...
    double current_score = best_score;
    fprintf(stderr, "initial_score : %lf\n", current_score);
    ...
    while (!sa.end()) {
        ...
        if (random::get(100) < 80) {
            int m = random::get(problem.musicians.size());
            geo::P& current_p = placements[m];
            double r = random::get_double(0.2, 1);
            double theta = random::get_double(0, 2 * M_PI);
            double dx = clamp(r * cos(theta), stage_left - current_p.X, stage_right - current_p.X);
            double dy = clamp(r * sin(theta), stage_bottom - current_p.Y, stage_top - current_p.Y);
            geo::P next_p = current_p;
            next_p.X += dx;
            next_p.Y += dy;
            bool ng = false;
            ...
            for (int i = 0; i < problem.musicians.size(); i++) {
                next_score -= ceil(q[i] * impact_sum[i]);
                next_score += ceil(tmp_q[i] * tmp_impact_sum[i]);
            }
            if (sa.accept(current_score, next_score)) {
                current_score = next_score;
                placements[m] = next_p;
            ...
            for (int musician : instrument[in1]) {
                ...
                tmp_q[m1] += new_dist;
                next_score -= ceil(q[musician] * impact_sum[musician]);
                next_score += ceil(tmp_q[musician] * impact_sum[musician]);
            }
            ...
            double is1 = 0, is2 = 0;
            next_score -= ceil(q[m1] * impact_sum[m1]);
            next_score -= ceil(q[m2] * impact_sum[m2]);
            for (int i = 0; i < problem.attendees.size(); i++) {
            ...
            next_score += ceil(tmp_q[m1] * is1);
            next_score += ceil(tmp_q[m2] * is2);
            if (sa.accept(current_score, next_score)) {
                current_score = next_score;
                swap(placements[m1], placements[m2]);
    ...
    placements = best_placements;
    double final_score = score_all_exact();
    fprintf(stderr, "best_score : %lf\n", final_score);
    fprintf(stderr, "diff_score : %lf\n", final_score - input_score);
