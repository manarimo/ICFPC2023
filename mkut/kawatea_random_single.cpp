...
void move(vector<geo::P>& placements, int idx, double dx, double dy, set<int>& moved) {
    if (moved.count(idx)) return;
    moved.insert(idx);
    placements[idx].first += dx;
    placements[idx].second += dy;
    if (placements[idx].first < stage_left) { moved.clear(); return; }
    if (placements[idx].first > stage_right) { moved.clear(); return; }
    if (placements[idx].second < stage_bottom) { moved.clear(); return; }
    if (placements[idx].second > stage_top) { moved.clear(); return; }
    for (int i = 0; i < placements.size(); i++) {
        if (i == idx) continue;
        if (dist2(placements[i], placements[idx]) < RADIUS2) {
            move(placements, i, dx, dy, moved);
