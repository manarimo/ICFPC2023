...
manarimo::atendee_t aggregated_attendee;
...
void input() {
    ...
    vector<double> taste_sum(problem.attendees[0].tastes.size(), 0);
    for (const auto attendee : problem.attendees) {
        xsum += attendee.x;
        ysum += attendee.y;
        for (int i = 0; i < attendee.tastes.size(); i++) {
            taste_sum[i] += attendee.tastes[i];
        }
    }
    aggregated_attendee = {xsum / problem.attendees.size(), ysum / problem.attendees.size(), taste_sum};
}
...
double score_one(const geo::P& p, int musician) {
    double sum = 0;
    sum += ceil(1000000 * aggregated_attendee.tastes[musician] / dist2(p, geo::P(aggregated_attendee.x, aggregated_attendee.y)));
    return sum;
}
